...
std::size_t init_thread_local_worker(std::size_t desired) {
	...
	if (current == desired) {
		...
		if (current == 0) {
			...
			std::cout << "Stencil maximal allowed half side-length is " << octotiger::fmm::STENCIL_WIDTH << " (Total length "
					<< 2 * octotiger::fmm::STENCIL_WIDTH + 1 << ")" << std::endl;
			std::cout << "Total number of stencil elements (stencil size): " << mono_inter::calculate_stencil().first.size() << std::endl << std::endl;
		}
		static_assert(octotiger::fmm::STENCIL_WIDTH <= INX, R"(
            ...
            Please increase either OCTOTIGER_THETA_MINIMUM or OCTOTIGER_WITH_GRIDDIM (see cmake file))");
				...
				std::cout << "OS-thread " << current << " on locality " << hpx::get_locality_id() << ": Initialized thread_local memory!\n";
				return desired;
			}
...
//	grid::set_unit_conversions();
		...
		std::array<size_t, 6> analyze_local_launch_counters() {
			...
			std::cout << "----------------------------------------" << std::endl;
			std::cout << "Total multipole launches on locality " << hpx::get_locality_id() << ": " << total_multipole_cpu_launches + total_multipole_cuda_launches
					<< std::endl;
			...
			std::cout << "----------------------------------------" << std::endl;
			std::cout << "Total p2p launches on locality " << hpx::get_locality_id() << ": " << total_p2p_cpu_launches + total_p2p_cuda_launches << std::endl;
			...
			if (total_p2p_cpu_launches + total_p2p_cuda_launches > 0) {
				float percentage = static_cast<float>(total_p2p_cuda_launches) / (static_cast<float>(total_p2p_cuda_launches) + total_p2p_cpu_launches);
		...
		void accumulate_distributed_counters() {
			...
			std::cout << "========================================" << std::endl;
			std::cout << "========================================" << std::endl;
			std::cout << "Total multipole launches: " << total_multipole_cpu_launches + total_multipole_cuda_launches << std::endl;
			...
			std::cout << "========================================" << std::endl;
			std::cout << "Total non-rho-multipole launches: " << total_multipole_cpu_launches_non_rho + total_multipole_cuda_launches_non_rho << std::endl;
     ...
     (_)
      ...
      `---' .'.''-._.-'`_./  /\ '.  \ _.-~~~````~~~-._`-.__.'
		...
		int main(int argc, char* argv[]) {
			...
			std::cout << "APEX Version : " << apex::version() << '\n';
