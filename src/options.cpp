...
bool options::process_options(int argc, char *argv[]) {
	...
	("dt_max", po::value<real>(&(opts().dt_max))->default_value(0.25), "max allowed pct change for positive fields in a timestep")           //
	...
	("multipole_kernel_type", po::value<interaction_kernel_type>(&(opts().m2m_kernel_type))->default_value(OLD), "boundary multipole-multipole kernel type") //
	...
	("p2m_kernel_type", po::value<interaction_kernel_type>(&(opts().p2m_kernel_type))->default_value(OLD), "boundary particle-multipole kernel type") //
