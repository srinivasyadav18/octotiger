...
//		diagnostics();
		...
		if (get_analytic() != nullptr) {
        		...
        		if (opts().gravity) {
		  		...
		  		auto duration =
				       	std::chrono::duration_cast<std::chrono::milliseconds>(stop_all_gravity - start_all_gravity);
		  		std::cout << "==> Overall execution time: " <<
				       	duration.count() << " ms" << std::endl;
		  		std::cout << "==> Average iteration execution time: " <<
				       	duration.count() / opts().stop_step << " ms" << std::endl;
			}
			if (!opts().disable_output) {
